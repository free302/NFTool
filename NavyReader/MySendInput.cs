...
using SC = NFT.NavyReader.ScanCodeShort;

namespace NFT.NavyReader
{
    using KD = IList<(bool isDown, int scan, int vk)>;
    public class MySendInput
    {
        #region ---- util ----
        static void build(SC[] keys, in INPUT[] inputs, ref int startIndex)
        {
            for (int i = 0; i < keys.Length; i++) inputs[startIndex++] = buildInput(keys[i]);
        }
        static INPUT buildInput(SC sc)
        {
            var ip = new INPUT();
            ip.type = 1; // 1 = Keyboard Input
            ip.U.ki.wScan = sc;
            ip.U.ki.dwFlags = KEYEVENTF.SCANCODE;
            return ip;
        }
        static INPUT buildInput(int vk, bool isDown)
        {
            var ip = new INPUT();
            ip.type = 1; // 1 = Keyboard Input
            ip.U.kir.KeyCode = (ushort)vk;
            ip.U.kir.Scan = 0;
            ip.U.kir.Time = 0;
            ip.U.kir.ExtraInfo = IntPtr.Zero;
        ...
        public static void SendKey(SC sc)
        {
            var inputs = new INPUT[1];
            inputs[0] = buildInput(sc);
            SendInput((uint)inputs.Length, inputs, INPUT.Size);
        }
        public static void SendKey(KD keys)
        {
            var inputs = new INPUT[keys.Count];
            for (int i = 0; i < keys.Count; i++) inputs[i] = buildInput((SC)keys[i].scan);
            SendInput((uint)inputs.Length, inputs, INPUT.Size);

            //var inputs = new INPUT[1];
            //for (int i = 0; i < keys.Count; i++)
            //{
            //    inputs[0] = buildInput(keys[i].vk, keys[i].isDown);
            //    SendInput((uint)inputs.Length, inputs, INPUT.Size);
            //    Thread.Sleep(10);
            //}
        }
        ...
        public static void SendString2(string text)
        {
            ...
            {
                var sc = (SC)(short)text[i]; ;
                var ip = new INPUT();
                ip.type = 1;
                ip.U.ki.wVk = 0;
                ip.U.ki.time = 0;
                ip.U.ki.wScan = sc;
                ip.U.ki.dwFlags = KEYEVENTF.UNICODE;
                ip.U.ki.dwExtraInfo = UIntPtr.Zero;
                inputs[0] = ip;
        ...
        public static void SendString(string text)
        {
            ...
            {
                var h = new VkKeyScanHelper { Value = VkKeyScan(chars[i]) };
                Debug.WriteLine($"h=({h.Low},{h.High}), keys={(Keys)h.Low}");

                var ip = new INPUT();
                ip.type = 1;
                ip.U.kir.KeyCode = h.Low;
                ip.U.kir.Flags = 0;
                inputs[2 * i] = ip;
    ...
    struct InputUnion
    {
        ...
        [FieldOffset(0)] internal KEYBDINPUT_RAW kir;
    }
    [StructLayout(LayoutKind.Sequential)]
    struct KEYBDINPUT
    {
    ...
    [StructLayout(LayoutKind.Explicit)] struct VkKeyScanHelper
    {
